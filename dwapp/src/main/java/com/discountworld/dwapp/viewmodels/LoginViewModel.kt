...
class LoginViewModel : ViewModel() {
    ...
    fun authenticateByPhone(phoneNumber: String, customerTier: String = "Gold") {
    ...
    fun authenticateByCnic(cnic: String) {
        val cleanCnic = cnic.replace("-", "").trim()
        if (cleanCnic.length < 13) {
            _authState.value = AuthState.Error("Please enter a valid 13-digit CNIC")
            return
        }
